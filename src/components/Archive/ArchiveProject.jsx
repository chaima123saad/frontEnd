...
const VISIBLE_FIELDS = ['_id', 'name', 'clientName', 'description', 'budget', 'priority', 'deletedAt'];
...
export default function DisableActionButtonsDataGridPro() {
  ...
  useEffect(() => {
    axios.get('http://localhost:2000/archives/6477197548c3cdc619664cbc')
      .then(response => {
        setData(response.data.deletedProjects);
        setFilteredData(response.data.deletedProjects);
      })
      .catch(error => {
        console.log(error);
      });
  ...
  return (
    <div style={{ height: '100vh', width: '100%' }} className='datatable'>
